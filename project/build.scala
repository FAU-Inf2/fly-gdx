...
object Settings {
  ...
  lazy val android = core ++ Tasks.natives ++ androidBuild ++ Seq(
    ...
    ) ++ Seq("armeabi", "armeabi-v7a", "x86").map { arch =>
        Seq("gdx", "gdx-bullet", "gdx-freetype").map { lib =>
          "com.badlogicgames.gdx" % s"$lib-platform" % libgdxVersion.value % "natives" classifier s"natives-$arch"
        }
    }.flatten,
    nativeExtractions <<= (baseDirectory) { base => 
      Seq("armeabi", "armeabi-v7a", "x86").map { arch =>
        (s"natives-$arch.jar", new ExactFilter("libgdx.so") | new ExactFilter("libgdx-bullet.so") | new ExactFilter("libgdx-freetype.so"), base / "libs" / arch)
      }
    },
    ...
    TaskKey[Unit]("emulate", "runs adb") in Android := {
      println("Running android build...")
      val apk = (packageDebug in Android).value
      println(s"Build complete. apk: $apk")
      Process("adb shell pm uninstall de.fau.cs.mad.fly.android") ! streams.value.log match {
        case 0 => "Uninstalled previous version."
        case _ => "No previous version found."
      }
      Process(s"adb install $apk") ! streams.value.log match {
        case 0 => "APK was installed."
        case _ => "Error installing the APK."
      }
      Process("adb shell am start -n de.fau.cs.mad.fly.android/de.fau.cs.mad.fly.android.AndroidLauncher") ! streams.value.log match {
        case 0 => "APK was installed."
        case _ => "Error installing the APK."
      }
      Process("adb -e logcat *:V")
    }
  )
  ...
  lazy val ios = core ++ Tasks.natives ++ Seq(
    ...
    unmanagedResourceDirectories in Compile += file("android/assets"),
    forceLinkClasses := Seq(
      "com.badlogic.gdx.scenes.scene2d.ui.*",
      "de.fau.cs.mad.fly.levels.**",
      ...
      "de.fau.cs.mad.fly.ios.sqlite.*",
      "de.fau.cs.mad.fly.ios.input.*",
      "de.fau.cs.mad.fly.features.ICollisionListener",
      "de.fau.cs.mad.fly.features.game.EndlessLevelGenerator",
      "de.fau.cs.mad.fly.features.game.EndlessRailLevelGenerator",
      "de.fau.cs.mad.fly.features.upgrades.types.Collectible",
      "de.fau.cs.mad.fly.game.GameObject",
      "de.fau.cs.mad.fly.player.Player",
      "de.fau.cs.mad.fly.profile.PlayerProfile",
      "de.fau.cs.mad.fly.res.GateDisplay",
      "de.fau.cs.mad.fly.res.GateGoal",
      "de.fau.cs.mad.fly.res.Perspective",
    ...
    libraryDependencies ++= Seq(
      ...
      "com.badlogicgames.gdx" % "gdx-bullet-platform" % libgdxVersion.value % "natives" classifier "natives-ios",
      "com.badlogicgames.gdx" % "gdx-freetype-platform" % libgdxVersion.value % "natives" classifier "natives-ios"
    ),
    nativeExtractions <<= (baseDirectory) { base => Seq(
      ("natives-ios.jar", new ExactFilter("libgdx.a") | new ExactFilter("libgdx-bullet.a") | new ExactFilter("libgdx-freetype.a") | new ExactFilter("libObjectAL.a"), base / "lib")
    )}
  )
}
  ...
  lazy val natives = Seq(
    ...
    extractNatives <<= (nativeExtractions, update) map { (ne, up) =>
      ...
      ne foreach { case (jarName, fileFilter, outputPath) =>
        jars filter(_.getName.contains(jarName)) map { jar =>
